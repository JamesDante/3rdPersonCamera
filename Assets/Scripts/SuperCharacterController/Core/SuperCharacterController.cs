...
public class SuperCharacterController : MonoBehaviour
{
    ...
    private List<IgnoredCollider> ignoredColliderStack = new List<IgnoredCollider>();
    ...
    void SingleUpdate()
    {
        if (currentGround == null) currentGround = new SuperGround(Walkable, this, triggerInteraction);
        ...
        if (collisionData != null) collisionData.Clear();
    ...
    public Vector3 IsOnTheWall(Vector3 dir)
    {
        if (dir != Vector3.zero)
        {
            foreach (var data in collisionData)
            {
                if (1 << data.gameObject.layer == Wall && Vector3.Dot(data.normal, dir) <= 0)
                {
                    return data.normal;
                }
            }
    ...
    void RecursivePushback(int depth, int maxDepth)
    {
        ...
        //var sumCol = Vector3.zero;

        foreach (var sphere in spheres)
        {
            var spherePosition = SpherePosition(sphere);
            //var sdss = Physics.OverlapSphere(spherePosition, radius, Walkable, triggerInteraction);
            foreach (Collider col in Physics.OverlapSphere(spherePosition, radius, Walkable, triggerInteraction))
            {
                ...
                //TODO fix fall to underground issue
                if (sphere.isFeet && contactPoint.y < transform.position.y) return;
    ...
    public class SuperGround
    {
        ...
        public bool doubleEdgeGrounded;
        ...
        public void ProbeGround(Vector3 origin, int iter)
        {
            ...
            if (Physics.SphereCast(o, smallerRadius, down, out hit, Mathf.Infinity, walkable, triggerInteraction))
            {
                ...
                {
                    RaycastHit oppseHit;
                    if (Physics.Raycast(hit.point, toCenter, out oppseHit, 2 * controller.radius, walkable, triggerInteraction))
                    {
                        var apDistance = Vector3.Distance(hit.point, controller.transform.position);
                        var oppDistance = Vector3.Distance(oppseHit.point, controller.transform.position);
                        if (Math.Abs(apDistance - oppDistance) < controller.radiusHalf)
                        {
                            doubleEdgeGrounded = true;
                        }
                    }
        ...
        public bool IsGrounded(bool currentlyGrounded, float distance, out Vector3 groundNormal)
        {
            ...
            if (stepGround != null && doubleEdgeGrounded) return true;
        ...
        private bool OnSteadyGround(Vector3 normal, Vector3 point)
        {
            ...
            //TODO add ratio for the distance
            float distanceFromCenter = Vector3.Distance(p, controller.transform.position) * 0.8f;
        ...
        private bool SimulateSphereCast(Vector3 groundNormal, out RaycastHit hit)
        {
            ...
            if (!Mathf.Approximately(groundAngle, 0))
            {
                ...
                //Debug.DrawLine(controller.transform.position, secondaryOrigin + controller.up * 2);
